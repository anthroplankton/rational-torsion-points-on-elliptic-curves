(function () {
	...
	class Polynomial extends Array {
	    ...
	    equal(other) {
	        const [shorter, longer] = Polynomial.sort(this, other);
	        return lodashExports.chain(longer)
	    ...
	    *rationalZeros() {
	    ...
	    newton(x, iteration = 1 << 12) {
	        for (let i = 0; i < iteration; ++i) {
	            const [f, df] = this.horner(x);
	            if (isclose(f, 0))
	                return x;
	            if (isclose(df, 0, 1e-8))
	                break;
	            const dx = f / df;
	            if (isclose(dx, 0))
	                return x;
	    ...
	    toString() {
	        const str = this.map((c, i) => c == 0n ? '' : `${c > 0n ? '+' : ''}${`${c == 1n && i > 0 ? '' : c}`}${i > 1 ? `x^{${i}}` : i > 0 ? 'x' : ''}`)
	            .reverse()
	            .join('')
	            .substring(1);
	        return str || '0';
	    }
	    static sort(...polynomials) {
...
//# sourceMappingURL=rational-zeros.js.map
